...
correlation_value_df = pd.DataFrame(index=all_vars, columns=all_vars, dtype=np.float64)
correlation_types_df = pd.DataFrame(index=all_vars, columns=all_vars, dtype=str)

# Create tabs
tab1, tab2, tab3 = st.tabs(["The Dataset", "IDA", "EDA"])
...
with tab2:
    ...
    st.info("You can choose which type of variables to display in the parallel coordinates graph. For numeric vatiables, you'll have the option to display them using z-scores, which normalize the data around a mean of 0 and a standard deviation of 1.")
    ...
    st.info("Histograms:  Below, you can customize a histogram. First, select the feature for the x-axis. Optionally, you can enable faceting to divide the histogram based on another feature.")
...
with tab3:
    ...
    This heatmap displays correlations between different variables. Dark blue or red indicates strong correlation, while white suggests little to no correlation. The correlation coefficient and its type are shown when you hover over each cell.
    
    
    Clicking on a cell will populate the scatter plot below with those variables.  
    ...
    for i, var1 in enumerate(all_vars):
        for j, var2 in enumerate(all_vars):
            if i > j:  # Only lower triangle
                ...
                correlation_types_df.loc[var1, var2] = corr_type  # Store the correlation type
                
                # Collect variables that used Spearman or Chi-square for special handling if needed
                if corr_type == 'S':
                    spearman_variables.add((var1, var2))  # Store variable pairs
                elif corr_type == 'C':
                    chi_square_variables.add((var1, var2))  # Store variable pairs
            else:
                correlation_value_df.loc[var1, var2] = np.nan
                correlation_types_df.loc[var1, var2] = None

    # Generate the hover text with both correlation values and types
    hovertext = [[f'{correlation_value_df.iloc[i, j]:.2f} ({correlation_types_df.iloc[i, j]})' if not pd.isna(correlation_value_df.iloc[i, j]) else ""
                  for j in range(len(all_vars))]
                 for i in range(len(all_vars))]

    with st.expander("Understanding the Correlation Matrix Tooltips"):
        st.info("""
        - Hover over the heatmap cells to see the correlation coefficients and types.
        - The correlation types are indicated as follows:
            - **P**: Pearson's correlation coefficient (used for linear relationships between two continuous variables), also used as the default for any categorical and numeric coeficient
            - **S**: Spearman's rank correlation coefficient (used for monotonic relationships between two continuous or ordinal variables)
            - **C**: Chi-square test of independence (used for relationships between two categorical variables)
        
        There is a rare error where the correlation heatmap will not appear if variables had been deslected in IDA tab, refreshing will 
        fix the heatmap.
        """)
    # Create the heatmap
    heatmap = go.Heatmap(
        z=correlation_value_df.to_numpy(),
        x=all_vars,
        y=all_vars,
        colorscale="RdBu_r",
        zmin=-1,
    ...
    fig.update_layout(title="Upper Triangle Correlation Matrix with Correlation Types")
    fig.update_traces(textfont=dict(size=9))  # Update to the desired font size
    ...
    combined_vars = multi_cat_vars + numeric_vars + binary_vars
    ...
    x_var_index = combined_vars.index(x_var_init)
    y_var_index = combined_vars.index(y_var_init)
